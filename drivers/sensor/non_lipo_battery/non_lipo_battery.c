...
#if IS_ENABLED(CONFIG_ZMK_SLEEP) && defined(CONFIG_ZMK_NON_LIPO_ADV_SLEEP_TIMEOUT) && (CONFIG_ZMK_NON_LIPO_ADV_SLEEP_TIMEOUT > 0)
...
static void check_voltage_and_shutdown(uint16_t millivolts) {
    ...
    if (millivolts <= CONFIG_ZMK_NON_LIPO_LOW_MV) {
        ...
        if (!zmk_usb_is_powered()) {
            LOG_WRN("Battery voltage (%dmv) below critical threshold (%dmv) and USB not connected",
